...
public class RecordDAO extends DAO {
	...
	public boolean addRecord(RecordDTO record) {
		boolean bool = false;
		try {
			conn = getConnection();
			String sql = "INSERT INTO records VALUES (" + "?," + "?," + "RECORD_SEQ.NEXTVAL," + "?)";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, record.getCharacterid());
			pstmt.setString(2, record.getMapno());
			pstmt.setInt(3, record.getCleartime());
	...
	public List<RecordDTO> getRecords(String userID, String characterName, String mapNO) {
		...
		try {
			...
			while (rs.next()) {
				rec.add(new RecordDTO(rs.getString("CHARACTERID"), String.valueOf(rs.getInt("MAPNO")),
						rs.getInt("RECORDNO"), rs.getInt("CLEARTIME"), rs.getString("CHARACTERNAME"),
	...
	public List<String> getClearRecords() {
		...
		try {
			...
			while (rs.next()) {
				clearRecordList.add("캐릭터명: " + rs.getString("CHARACTERNAME") + "\t맵명: " + rs.getString("mapname")
						+ "\t클리어시간: " + rs.getString("cleartime"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public List<String> getUserClearCounts() {
		...
		try {
			...
			while (rs.next()) {
				clearCountList.add("계정ID: " + rs.getString("userid") + "\t맵명: " + rs.getString("mapname") + "\t클리어횟수: "
	...
	public List<String> getCharacterRankList(CharacterDTO character) {
		...
		try {
			...
			while (rs.next()) {
				clearCountList.add("맵명: " + rs.getString("mapname") + "\t순위: " + rs.getString("rank") + " 위");
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public List<RecordDTO> getRecordList(String id) {
		...
		try {
			conn = getConnection();
			String sql = "select * " + "from records r,users u,characters c,maps m "
			...
			rs = pstmt.executeQuery();
			while (rs.next()) {
				RecordList.add(new RecordDTO(rs.getString("characterID"), rs.getString("mapno"), rs.getInt("recordNo"),
						rs.getInt("clearTime"), rs.getString("charactername"), rs.getString("mapname")));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public List<RecordDTO> getCountRecordList(String id) {
		...
		try {
			conn = getConnection();
			String sql = "SELECT" + "    M.mapname, count(*) AS clearCount" + " FROM"
					+ "    records R, characters C, maps M\r\n" + " WHERE" + "    R.characterID = C.characterID"
					+ "    AND R.mapno = M.mapno and c.userid=?" + " GROUP BY M.mapname";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public List<RecordDTO> getSkillRecordList(String id) {
		...
		try {
			conn = getConnection();
			String sql = "SELECT" + "    S.skillname, count(*) AS ClearCount" + " FROM"
					+ "    records R, characters C, skills S" + " WHERE" + "    R.characterID = C.characterID"
					+ "    AND C.skillID = S.skillID" + "    and c.userid=?" + "group by" + "    S.skillName";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public int getRank(String mapNo, String characterID) {
		try {
			conn = getConnection();
			String sql = "SELECT COUNT(*)+1 AS RANK FROM (SELECT min(clearTime) as minClearTime FROM records WHERE mapNo = ? GROUP by CHARACTERID)"
					+ "WHERE minClearTime < (SELECT min(CLEARTIME) FROM RECORDS WHERE MAPNO = ? AND CHARACTERID = ?)";
			pstmt = conn.prepareStatement(sql);
